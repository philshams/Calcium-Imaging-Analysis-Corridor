...
% if not filtering by activity, include all cells    
    ...
    cells_to_use{session} = 1:size(position_response_array_multi_session.dff{session},2);
...
for session = 1:num_sessions
    position_response_array_multi_session.dff_active{session} = position_response_array_multi_session.dff{session}(:,cells_to_use,:);
end