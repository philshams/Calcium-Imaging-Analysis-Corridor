...
% make figure
if  session == 1
z_figure = figure('Name',figure_name,'Position', [29 704 1865 439]); hold on; movegui(gca,'onscreen')
set(z_figure,'color','black'); z_figure.InvertHardcopy = 'off';
else
    figure(z_figure)
end

z_axes(end+1) = subplot(1,num_sessions,session); hold on
...
title(['Responsiveness by preferred location in corridor -- session ' num2str(session)],'color','w')
